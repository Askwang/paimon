...
class PaimonAnalysis(session: SparkSession) extends Rule[LogicalPlan] {
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsDown {
    ...
    // batches = Batch("Resolution", spark rules ++ extendedResolutionRules)
    // PaimonAnalysis 是在 spark rule 解析后增加的规则，ResolvePartitionSpec 规则还是会生效，
    // 所以解析 DropPartition 还得提前，放在 AstBuilder parser 中解析，即 injectParser
